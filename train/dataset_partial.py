...
class DFRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        un =  self.cfg["unsupervised"]
        Troot =  self.cfg["datasetname"]
        if un is True:
            #vts = tuple(i for i in range(0, verts.size()[0]))
            if self.cfg["datasetname"] == "scape" or self.cfg["datasetname"] == "faust" or self.cfg["datasetname"] == "TOSCA" or self.cfg["datasetname"] == "stomach_r" :
                T21_name = 'T_' + name1 + '_' + name2 + '.txt'
                T12_name = 'T_' + name2 + '_' + name1 + '.txt'
                T21 = np.loadtxt(os.path.join('../data/'+ Troot+'/T',T21_name)) - 1
                T12 = np.loadtxt(os.path.join('../data/'+ Troot+'/T',T12_name)) - 1
            else:
                T12 = np.arange(0, self.verts_list[idx2].size()[0])
                T21 = np.arange(0, self.verts_list[idx1].size()[0])
            T21 = torch.tensor(T21).long()
            T12 = torch.tensor(T12).long()
...
def shape_to_device(dict_shape, device):
    names_to_device = ["verts", "phi_partial","eval", "phi","verts_sample","fps_sample"]
