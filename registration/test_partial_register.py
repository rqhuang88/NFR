...
from utils import read_geodist, calculate_geodesic_error, compute_geodesic_distmat, search_t, compute_vertex_normals
...
for numview in range(12):
    if not os.path.exists(os.path.join(save_path,cfg["save_name"],cfg["save_name"]+'_view_'+str(numview+1))):
        os.makedirs(os.path.join(save_path,cfg["save_name"],cfg["save_name"]+'_view_'+str(numview+1)))
    with open(os.path.join(save_path,cfg["save_name"],cfg["save_name"]+'_view_'+str(numview+1),'config.yaml'), "w") as f:
            yaml.dump(cfg, f)
    with torch.no_grad():
    ...
    for i, data in tqdm(enumerate(test_loader)):
        ...
        pth = partial_path + "/index_" + name2 +"_view_" + str(numview+1)+ ".txt"  
        print(pth)
        ...
        #     rotated_normal = torch.matmul(target_normal, rotation_matrix)
        #     idx_partial = torch.from_numpy(np.asarray(np.where(rotated_normal[:,2]>0))).long().squeeze()
        #     if idx_partial.shape[0] > target_mesh_V.shape[0]*0.7:
        #     #if idx_partial.shape[0] > 2200:
        #         break



        # dataset_path_test-get partial index
        # num = cfg["exp_num"]
        # lis = [-60,-30,0,30,60]
        # angle = (math.floor((int(num)-1)/5))*72
        # while True:
        #     #angle = 360*random.random()
        #     radian = torch.deg2rad(torch.tensor(angle))
        #   
        ...
        #                 [0.0, 1.0, 0.0],
        #                 [-torch.sin(radian), 0.0, torch.cos(radian)]], dtype=torch.float32)
        #     rotate_normal_level = torch.matmul(target_normal, rotation_matrix)
            
        #     angle_sphere = np.radians(lis[(int(num)-1)%5])
        #   
        #     R_vertical = np.array([[1, 0, 0],
        #                             [0, np.cos(angle_sphere), -np.sin(angle_sphere)],
        #                             [0, np.sin(angle_sphere), np.cos(angle_sphere)]])
        #     R_vertical = torch.tensor(R_vertical, dtype=torch.float32)
        #     rotated_normal = torch.matmul(rotate_normal_level, R_vertical)

        #     idx_partial = torch.from_numpy(np.asarray(np.where(rotated_normal[:,2]>0))).long().squeeze()
        #     #if idx_partial.shape[0] > target_mesh_V.shape[0]/2:
        #     if idx_partial.shape[0] > 2200:
        #         break
        #     angle += 10
        # print(angle)
        # print(lis[(int(num)-1)%5])
        target_mesh_V = target_mesh_V[idx_partial]
        ...
        # save_path_t = os.path.join(save_path ,cfg["save_name"], f'points')
        # if not os.path.exists(save_path_t):
        #     os.makedirs(save_path_t)
        # o3d.io.write_point_cloud("target_pcd_"+name2+".ply", target_pcd)
        ...
        with torch.no_grad():
            ...
            # save_path_t = os.path.join(save_path,cfg["save_name"] ,cfg["save_name"]+'unregister_view_'+str(numview+1), f'T')
            # if not os.path.exists(save_path_t):
            #     os.makedirs(save_path_t)
        ...
        while True:
            if idx > 10000:
                save_path_t = os.path.join(save_path,cfg["save_name"] ,cfg["save_name"]+'_view_'+str(numview+1), f'T')
                if not os.path.exists(save_path_t):
                    os.makedirs(save_path_t)
                filename_t12 = f'T_{name1}_{name2}.txt'
                t12 = T12_pred.detach().cpu().squeeze(0).numpy()
                ...
                save_path_mesh = os.path.join(save_path,cfg["save_name"] ,cfg["save_name"]+'_view_'+str(numview+1), f'mesh')
                ...
                save_path_t = os.path.join(save_path,cfg["save_name"] ,cfg["save_name"]+'_view_'+str(numview+1), f'index_partial')
                if not os.path.exists(save_path_t):
                    os.makedirs(save_path_t)
                filename_index = f'index_{name2}.txt'
                idx1p = idx_partial.detach().cpu().squeeze(0).numpy()
                np.savetxt(os.path.join(save_path_t, filename_index), idx1p, fmt='%i')
                all_time = time.time()
                all_time_rec = all_time - start_time
                print('total time:',all_time_rec)
            ...
            time_geo_start = time.time()  
            Geo_dist_idx = geod[S_idx.cpu().numpy(),S_temp2.cpu().numpy()] < 0.001
            time_geo_end = time.time()
            corr_filter_time = corr_filter_time + (time_geo_end-time_geo_start)
            Geo_dist_idx = torch.from_numpy(Geo_dist_idx).to(device)
            

            mesh_deform_start_time = time.time()
            new_source_mesh_V,arap,sr_loss= dg(source_mesh_V,opt_d_rotations, opt_d_translations)
            mesh_deform_end_time = time.time()
            mesh_time = mesh_time + (mesh_deform_end_time - mesh_deform_start_time)
            ...
            energy_time = energy_time + (energy_end_time-energy_start_time)
            ...
            if idx == 0:
                ...
                if count > 15:
                    ...
                    if using_xyz is False:
                        ...
                        stage1_time = end_time_stage1-start_time
                        ...
                        print('correspondence time:',corr_filter_time)
                        print('mesh_deforming time:',mesh_time)
                        print('energy_computing time:', energy_time)
                        print('updating time:',corr_time)
            ...
            if idx % 100 == 0:
                ...
                corr_time = corr_time + (updata_end_time-updata_start_time)
            ...
            admm_time = admm_time + (admm_end_time - admm_start_time)
