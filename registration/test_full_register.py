...
for numview in range(49,50):
    ...
    for i, data in tqdm(enumerate(test_loader)):
        ...
        target_shape_num  = target_mesh_V.shape[0]
        if target_shape_num <= 20000:
            print("size too small,continue")
            continue
        ...
        pth = partial_path + "/" + name2.replace("reg", "scan") +"_view" + str(numview+1)+ ".txt"  
        print(pth)
        ...
        # if target_shape_num > 20000:
        #     target_mesh_V_ori = target_mesh_V
        #     target_mesh_V = target_mesh_V[target_mesh_V_fps.squeeze()[:3000]]
        #     V2 = target_mesh_V.unsqueeze(0).to(device) # 2514 3
        #     V2_fps_sample =torch.arange(0,1500).long().to(device).unsqueeze(0) # 1500
        #     V2_sample = V2[0,:1500].unsqueeze(0).to(device) # 1500 3
        #     print(V2.shape,V2_fps_sample.shape,V2_sample.shape)
        # else:
        V2 = target_mesh_V.unsqueeze(0).to(device) # 2514 3
...
#                 if target_shape_num > 15000:
#                     T12_pred = search_t(new_source_mesh_V, target_mesh_V_ori) 
#                     T21_pred = search_t(target_mesh_V_ori, new_source_mesh_V) 
