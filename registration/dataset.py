    ...
    def __init__(self, root_dir, name="remeshed",use_cache=True,train=True,single=False,faustdata = False):
        ...
        if use_cache:
            ...
            if os.path.exists(load_cache):
                ...
                if faustdata is True:
                    self.combinations = list(permutations(range(70,len(self.used_shapes)), 2))
                elif faustdata == 'shrec19tem':
                    self.combinations = list(permutations(range(38,len(self.used_shapes)), 2))
                elif faustdata == 'shrec19':
                    self.combinations = [(0, j) for j in range(0, 44)]
                    print(self.combinations)
                elif faustdata == 'shrec07':
                    self.combinations = [(0, j) for j in range(0, 20)]
                    print(self.combinations)
                elif single is False:
                    self.combinations = list(permutations(range(len(self.used_shapes)), 2))
                else:
                    numb = 0
                    #self.combinations = [(10,10)]
