...
for root, dirs, files in os.walk('realscans/shapes_ply/'):
    for file in files:

        target_pcd = o3d.geometry.PointCloud()
        pth0 = os.path.join(root, file)
        #target_mesh_V = data["shape2"]['vts'].squeeze()  # 5000 3
        pcd = pp3d.io.read_point_cloud(pth0)
        target_mesh_V = torch.tensor(np.ascontiguousarray(pcd)).float()
        name2=os.path.splitext(file)[0]
        # print(target_mesh_V)
        # pth = "realscans/index_partial/"+name2+"_fps.txt"
        # print(pth)
